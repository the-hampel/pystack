...
parser.add_option("--occupancies",dest="occupancies",help="""Supply concentrations of elements. Must match the elements in POSCAR. Example: --occupancies="1.0,0.5,0.5" """,metavar="list of occupanciess")
parser.add_option("--displacement-vector",dest="displacementvector",help="""A vector in relative coordinates with which to displace all atoms before outputting to .cif file.""",metavar="[x,y,z]")
parser.add_option("--findsym-tolerance",dest="findsymtol",help="""Tolerance used for FINDSYM (default=0, minimal value).""")
parser.add_option("--no-findsym",dest="nofindsym",help="""Don't run FINDSYM to find symmetry of the crystal.""",action="store_true")
	...
	if filetype(input_file) == "POSCAR":
		...
		if not options.occupancies:
			if os.path.exists("INCAR"):
				incar_lines = commands.getoutput("grep VCA INCAR").split("\n")
				for line in incar_lines:
					if re.search("VCA[ ]*=",line.split('#')[0]):
						occupancies = [float(x) for x in line.split('#')[0].split("=")[1].split()]
		if not occupancies:
		...
		for i in range(0,len(atomlabels)):
			...
			cell.sites.append((atomlabels[i][0],coords[0],coords[1],coords[2],atomlabels[i][1]))
		...
		# Check for VCA calculation
		if not options.occupancies:
			vcalines = commands.getoutput("grep '   VCA    =' "+filename).split("\n")
		...
		for line in input_file:
			...
			if 0 <= posline < sum(natoms):
				...
				cell.sites[posline] = (cell.sites[posline][0],c[0],c[1],c[2],cell.sites[posline][4])
